      ...
      my_baudrate(DEFAULT_BAUDRATE)
{
    ...
    if (this->my_serial_file_descriptor < 0)
    {
        // There was an error
    ...
    terminal_configuration.c_cflag =
        static_cast<tcflag_t>(CS8 | CLOCAL | CREAD | [](BAUDRATE baud) {
            switch (baud)
            {
...
void SERIAL_PORT_WRAPPER::setBaudRate(SERIAL_PORT_WRAPPER::BAUDRATE /* baud */)
{
    // Currently not supporting changing the baudrate
    // TODO: Add configuration to support baud rate.
    assert(false);
}
